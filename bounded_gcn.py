...
class GraphSAGE(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        if self.aggr_method == 'mean':
            ...
            neighbor_mean = torch.sparse.FloatTensor(row, col, torch.ones_like(row, dtype=torch.float32), size=tuple(x.shape[0], x.shape[0])).to(x.device)
            ...
            neighbor_max = torch.sparse.FloatTensor(row, col, torch.ones_like(row, dtype=torch.float32), size=(x.shape[0], x.shape[0])).to(x.device)

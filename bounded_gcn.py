...
class BoundedGCN(nn.Module):
    ...
    def __init__(self, nfeat, nhid, nclass, dropout=0.5, lr=0.01, weight_decay=5e-4,
        ...
        self.nclass = nclass
        self.sg1 = SAGEConv(nfeat, nhid, bias=with_bias)
        self.sg2 = SAGEConv(nhid, nclass, bias=with_bias)
    ...
    def initialize(self):
        ...
        """
        self.sg1.reset_parameters()
        self.sg2.reset_parameters()
    ...
    def fit(self, features, adj, labels, idx_train, idx_val=None, train_iters=200, initialize=True, verbose=False, normalize=True, patience=500, **kwargs):
        ...
        self.device = self.sg1.lin_l.weight.device
        if initialize:
            self.initialize()
    ...
    def _train_without_val(self, labels, idx_train, train_iters, verbose):
        ...
        for i in range(train_iters):
            for batch in self.train_loader:
                ...
                self.l2_reg = self.bound * torch.square(torch.norm(self.sg1.lin_l.weight)) + torch.square(torch.norm(self.sg2.lin_l.weight))  # Added by me

                # print(f'L2 reg at iteration {i} = {l2_reg}')
                loss_train = F.cross_entropy(output[idx_train], labels[idx_train]) + self.bound*self.l2_reg
                loss_train.backward()
                optimizer.step()
    ...
    def _train_with_val(self, labels, idx_train, idx_val, train_iters, verbose):
        ...
        for i in range(train_iters):
            for batch in self.train_loader:
                ...
                self.l2_reg = 2 * self.bound * (torch.log(torch.norm(self.sg1.lin_l.weight)) + torch.log(torch.norm(self.sg2.lin_l.weight)) )    # Added by me
